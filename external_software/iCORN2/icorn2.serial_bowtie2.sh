...
# Parallel processing of the sequences...
	...
	if [ $low_mem_mode == "no" ]; then
		...
		parallel --verbose --joblog processing_split_parts_correction_icorn2_log_out_2.txt -j $parallel_block_size icorn2.snpcall.correction.sh {} $cores_split $readRoot $fragmentSize {}.$(($i+1)) $i "&>" {}.snpcall.correction.log ::: ${arr[@]} &> processing_split_parts_correction_icorn2_log_out.txt
		...
		if [ "$(awk '{ print $9 }' processing_split_parts_correction_icorn2_log_out_2.txt | grep -c "9")" -gt 0 ]; then
			...
			parallel --verbose --joblog processing_split_parts_correction_icorn2_log_out_2_2.txt -j $parallel_block_size icorn2.snpcall.correction.sh {} $cores_split $readRoot $fragmentSize {}.$(($i+1)) $i "&>" {}.snpcall.correction.log ::: ${arr[@]} &> processing_split_parts_correction_icorn2_log_out2.txt
		...
		echo -e "Low memory mode, sequential processing instead of parallel, cores to be used to process each split sequence: $((cores / 2))"
		for part in $(find . -name "*.fa" -exec basename {} \; | grep -v ref.fa); do
			...
			icorn2.snpcall.correction.sh $part $part.bam $((cores / 2)) $readRoot $fragmentSize $part.$(($i+1)) $i
		done
	fi
	cd ../
