...
if [[ $mode == "blast" || $mode == "taxon" || $mode == "both" ]]; then
	...
	if [[ -f $databases/taxdump/names.dmp ]] && [[ -f $databases/taxdump/nodes.dmp ]] && [ "$(find -L $databases -name hash.k2d | wc -l)" -gt 0 ]; then
		...
		echo -e "\n\nAlternatively, you can download the already built database: cd $databases && wget --no-check-certificate" '"bit.ly/kraken2_dbs" -O standard_eupathdb_48_kraken2_db.tar.xz && tar -xf standard_eupathdb_48_kraken2_db.tar.xz && rm standard_eupathdb_48_kraken2_db.tar.xz'
...
if [[ $debug == "all" || $debug == "step2a" ]]; then
	...
	if [ $cores -ge $blocks_size ]; then
		...
		if [ "$(awk '{ print $9 }' megablast_parallel_log_out_2.txt | grep -c "9")" -gt 0 ]; then
			...
			cores_split=$((cores / 3))
			parallel --verbose --joblog megablast_parallel_log_out_2_2.txt -j 3 megablast -W 40 -F F -a $cores_split -m 8 -e 1e-80 -d $dir/1.Filtering/01.assembly.fa -i {} -o comp.self1.{}.blast ::: ${arr[@]} &> megablast_parallel_log_out2.txt
