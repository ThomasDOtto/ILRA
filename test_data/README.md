...
ILRA.sh -a assembly_Pf_test.fasta -o out_ILRA_test2 -c corrected_reads_Pf_test_subset.fastq.gz -n test -r PlasmoDB-47_Pfalciparum3D7_Genome_core_PMID_29862326.fasta -I Illumina_short_reads_Pf_test_subset -t 4 -g PlasmoDB-50_Pfalciparum3D7.gff -L pb -M 32g 2>&1 | tee -a out_ILRA_test2_log.txt
```
...
Please go through the output file 'out_ILRA_test_log.txt' and the folder 'out_ILRA_test' to get the details on the example run of pipeline and the final output. Here, we provide the following overview:
