...
### Parameters are not positional. If you did not provide any required parameter, the pipeline will exit or use default values if possible (check log or 'Arguments / Variables' section in the pipeline ILRA.sh)
...
### Many software and dependencies are also required and are going to be automatically checked. The pipeline will exit if any required software is not found in the variable PATH of your system, which you likely need to change accordingly. Check 'Checking the installed software and databases' section in the pipeline ILRA.sh.

######### Databases for decontamination:
### The subfolder databases must exist and it should be populated manually by the user before executing ILRA. ILRA is going to provide instructions, but check if needed the 'Checking the installed software and databases' section in the pipeline ILRA.sh.
